...
export function ProductPhotoIdentifier({ onIdentified }: Props) {
  ...
  const analyzePhoto = async (file: File) => {
    setLoading(true);
    try {
      const data = await api.identifyFromProductPhoto(file);
      setResult(data);
      if (data.brand || data.model) {
        onIdentified({
          ...data,
          found: true,
          identificationMethod: 'product-photo',
        });
      }
    } catch {
    ...
    <div className="space-y-4">
      ...
      {!preview ? (
        ...
        >
          ...
          <span className="text-zinc-400">{t('identify.captureProduct')}</span>
        </button>
      ) : (
        <div className="space-y-3">
          <img src={preview} alt="Product" className="w-full max-h-48 object-contain rounded-lg border border-border" />
          {loading && (
          ...
          {result && !loading && (
            <div className="rounded-lg border border-border bg-dark-tertiary p-3 space-y-1">
              <p className="text-xs text-zinc-500">{t('identify.productIdResult')}</p>
              ...
              {result.category && <p className="text-sm text-white">{t('review.category')}: {result.category}</p>}
              {result.confidence && (
